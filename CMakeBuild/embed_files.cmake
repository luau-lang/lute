...
file(READ \"${abs_file}\" file_content HEX)
string(LENGTH \"\${file_content}\" content_length)
math(EXPR num_bytes \"\${content_length} / 2\")

set(escaped_content \"\")
set(i 0)
while(i LESS num_bytes)
    math(EXPR hex_start \"\${i} * 2\")
    math(EXPR hex_end \"\${hex_start} + 1\")
    string(SUBSTRING \"\${file_content}\" \${hex_start} 2 hex_byte)
    set(escaped_content \"\${escaped_content}\\\\x\${hex_byte}\")
    math(EXPR i \"\${i} + 1\")
endwhile()

file(APPEND \"${EMBED_OUTPUT_HEADER}\" \"    {\\\"${EMBED_PATH_PREFIX}${rel_key}\\\", \\\"\${escaped_content}\\\"}${has_comma}\\n\")
")
        math(EXPR idx "${idx} + 1")
    endforeach()
