...
`lute debug serve` connects Luau code to development environments via the Debug Adapter Protocol, allowing for easy debugging. Features supported include breakpoints, stepping, expression evaluation, and more. Additionally, you can use the `@lute/debugger` library to write your own personal debugging scripts!gi For more information, check out the [debug documentation](../../cli/debug/debug.md).
