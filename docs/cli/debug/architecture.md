...
Threads, stack frames, variable scopes/variables all receive their own set of unique IDs. Variable scopes and variables receive the same ID space, while threads and stack frames have their own isolated ID space. These IDs helps us identify what object we are inspecting into. For example, we pass in a thread ID to inform `Target::getStackTrace()` of which thread we are trying to get the stack trace from. Coroutine receives a unique ID number that we use to track it across its lifetime. The other objects have ID numbers that are only unique to the current suspended state. Thus, these ID numbers will get reset if the `Target` is continued. For further explanation, see "Lifetime of Object References" in the [DAP overview](https://microsoft.github.io/debug-adapter-protocol/overview).
