...
For example, before launching a debuggee, you can `setBreakpoint` and `removeBreakpoint`. When launching a debuggee, you can configure callbacks like `onBreakpointHit` to be informed when certain events occur on the debuggee. Within these callbacks, you can then perform a variety of actions. For example, when the code is paused, you are able to:
...
After pausing code, remember to run `continueProcess` to restart execution.
