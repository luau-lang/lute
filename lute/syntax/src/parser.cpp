...
struct AstSerialize : public Luau::AstVisitor
{
    ...
    std::vector<Trivia> extractWhitespace(const Luau::Position& newPos)
    {
        ...
        while (!trivia.empty())
        {
            ...
            result.push_back(Trivia{Trivia::TriviaKind::Whitespace, Luau::Location{beginPosition, currentPosition}, part});
    ...
    std::vector<Trivia> extractTrivia(const Luau::Position& newPos)
    {
        ...
        for (const auto& comment : comments)
        {
            ...
            auto kind = comment.type == Luau::Lexeme::Comment ? Trivia::TriviaKind::SingleLineComment : Trivia::TriviaKind::MultiLineComment;
    ...
    static std::pair<std::vector<Trivia>, std::vector<Trivia>> splitTrivia(std::vector<Trivia> trivia)
    {
        ...
        {
            if (trivia[i].kind == Trivia::TriviaKind::Whitespace && trivia[i].text.find('\n') != std::string::npos)
                break;
        }
    ...
    void serializeTrivia(const std::vector<Trivia>& trivia)
    {
        ...
        for (size_t i = 0; i < trivia.size(); i++)
        {
            ...
            {
            case Trivia::TriviaKind::Whitespace:
                lua_pushstring(L, "whitespace");
                break;
            case Trivia::TriviaKind::SingleLineComment:
                lua_pushstring(L, "comment");
                break;
            case Trivia::TriviaKind::MultiLineComment:
...
static int initSyntaxParserLibrary(lua_State* L)
{
...
const luaL_Reg SyntaxParser::lib[] = {{"parse", luau::luau_parse}, {"parseExpr", luau::luau_parseexpr}, {nullptr, nullptr}};

const char* const SyntaxParser::properties[] = {"cst", luau::kSpanType};

int SyntaxParser::pushLibrary(lua_State* L)
{
    lua_createtable(L, 0, std::size(SyntaxParser::lib) + std::size(SyntaxParser::properties));

    for (auto& [name, func] : SyntaxParser::lib)
    {
        if (!name || !func)
            break;
    ...
    return luau::initSyntaxParserLibrary(L);
}

LUTE_API int luaopen_syntax_parser(lua_State* L)
{
    return SyntaxParser::openAsGlobal(L);
}

LUTE_API int luteopen_syntax_parser(lua_State* L)
{
    return SyntaxParser::pushLibrary(L);
}