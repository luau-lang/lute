...
void FSReadIntoBuffer::readCallback(uv_fs_t* req)
{
    auto r = uvutils::retake<FSReadIntoBuffer>(req);
    auto bytesRead = req->result;
    ...
    r->succeed(
        [bytesRead](lua_State* L)
        {
            lua_pushnumber(L, static_cast<double>(bytesRead));
            return 1;
        }
    );
}
